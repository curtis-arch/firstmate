...
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/fm-meta-lib.sh"
...
fmx_meta_link_set() {
  ...
  [ "$status" -ne 0 ] || printf 'x_request=%s\n' "$rid" >> "$tmp" || status=$?
  [ "$status" -ne 0 ] || printf 'x_request_ts=%s\n' "$ts" >> "$tmp" || status=$?
  [ "$status" -ne 0 ] || printf 'x_followups=%s\n' "$followups" >> "$tmp" || status=$?
  ...
  if [ "$status" -eq 0 ]; then
    case "$reply_max" in
      ''|*[!0-9]*) ;;
      *) printf 'x_reply_max_chars=%s\n' "$reply_max" >> "$tmp" || status=$? ;;
    esac
  fi
  [ "$status" -ne 0 ] || mv -f "$tmp" "$meta" || status=$?
...
fmx_meta_followups_set() {
  ...
  if [ "$status" -eq 0 ] && ! { grep -vE '^x_followups=' "$meta" || true; } > "$tmp"; then status=1; fi
  [ "$status" -ne 0 ] || printf 'x_followups=%s\n' "$n" >> "$tmp" || status=$?
  [ "$status" -ne 0 ] || mv -f "$tmp" "$meta" || status=$?
  [ "$status" -eq 0 ] || rm -f "${tmp:-}"
  fm_meta_lock_release "$meta"
