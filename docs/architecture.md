...
`fm-watch.sh` polls each window's backend for a busy state: Orca joins the exact recorded terminal and worktree to its native agent inventory, herdr consults `agent.get`, and tmux, zellij, and cmux report unknown; every unknown native result falls back to the existing pane-tail-regex detection.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and herdr have verified classifiers for secondmate recovery, Orca has an exact classifier but does not support secondmate spawns, and zellij plus cmux report `unknown` rather than guess.
...
Orca is experimental and selected only explicitly: Orca owns both worktree and terminal lifecycle, records durable `orca_worktree_id=` plus `orca_pane_key=` beside the runtime-epoch `terminal=` handle, detects a runtime-confirmed missing worktree independently of endpoint liveness, and removes worktrees through `orca worktree rm` only after the usual firstmate teardown checks pass. Its current behavior and limitations are recorded in `docs/orca-backend.md`.
Orca semantic liveness is shipped from the 2026-07-14 E1b contract: the exact recorded worktree plus `terminal show`'s unique `tabId:leafId` join to exactly one `worktree ps --json` agent.
Only observed `working`, turn-complete `done`, and connected-shell agent disappearance are mapped; every ambiguous or unreadable result remains `unknown`, and watcher consumers retain their existing terminal-tail fallback.
