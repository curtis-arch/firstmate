...
Orca semantic liveness remains thin after the 2026-07-12 E1 refusal: no verified relation connects Firstmate's recorded `terminal=term_...` handle to `worktree ps --json` `agents[].paneKey`, so busy state and agent liveness remain `unknown` and consumers retain their existing fallback policy.
Fixture coverage pins that fail-closed result, but only a future disposable Firstmate-created scout experiment can unlock the semantic mapping.
