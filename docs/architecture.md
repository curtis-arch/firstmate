...
Orca is experimental and selected only explicitly: Orca owns both worktree and terminal lifecycle, records durable `orca_worktree_id=` plus `orca_pane_key=` beside the runtime-epoch `terminal=` handle, and removes worktrees through `orca worktree rm` only after the usual firstmate teardown checks pass. Its current behavior and limitations are recorded in `docs/orca-backend.md`.
