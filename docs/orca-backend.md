...
- `fm-watch.sh` joins the recorded terminal to Orca's native per-agent state through `terminal show` and `worktree ps`.
  A verified `working` agent is busy and a verified turn-complete `done` agent is idle; every unobserved state retains the existing terminal-tail fallback.
- `fm-crew-state.sh` reads the recorded Orca terminal when no no-mistakes run-step applies.
  Native agent-state handling is limited to the observed `working`, `done`, and empty-agent shapes.
...
- `waiting` and `blocked` remain unrecognized and normalize to `unknown` because a bounded live probe could not produce either state deterministically.
...
### Unobserved attention states - rejected (2026-07-15)
...
- rejection of unobserved `waiting` and `blocked` rows as unknown semantic state and unknown liveness;
