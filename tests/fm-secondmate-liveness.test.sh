...
test_meta_process_stamp_is_locale_invariant() {
  local fb log out
  fb=$(fm_fakebin "$TMP_ROOT/meta-stamp-locale")
  log="$TMP_ROOT/meta-stamp-locale/locale.log"
  cat > "$fb/ps" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "${LC_ALL:-unset}" > "${FM_TEST_LOCALE_LOG:?}"
printf '%s\n' 'Wed Jul 15 12:34:56 2026'
SH
  chmod +x "$fb/ps"

  out=$(PATH="$fb:$BASE_PATH" FM_TEST_LOCALE_LOG="$log" bash -c \
    '. "$0/bin/fm-meta-lib.sh"; fm_meta_process_stamp "$$"' "$ROOT")

  [ "$out" = WedJul151234562026 ] || fail "unexpected teardown process stamp: $out"
  [ "$(cat "$log")" = C ] || fail "fm_meta_process_stamp did not pin LC_ALL=C"
  pass "fm_meta_process_stamp: teardown owner identity is locale-invariant"
}
...
test_sweep_respawns_confirmed_dead_secondmate() {
  ...
  assert_not_contains "$(cat "$w/home/state/sm1.meta")" "lifecycle=" \
    "a successful respawn must atomically clear its replacement reservation"
...
test_replacement_failure_cleans_new_endpoint_and_stale_metadata() {
  ...
  [ "$status" -ne 0 ] || fail "replacement should fail when task temp setup is blocked"
  assert_contains "$out" "Not a directory" "replacement failure did not reach the injected setup error"
  assert_contains "$(cat "$log")" "new-window" "replacement fixture did not create a new endpoint"
