...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  assert_not_contains "$info_line" "sm-readme" "readme-only advance is not nudged"
  assert_not_contains "$info_line" "sm-current" "already-current secondmate is not nudged"
...
test_bootstrap_nudge_retry_is_idempotent() {
  ...
  out2=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$w/home" FM_ROOT_OVERRIDE="$w/main" \
    FM_SEND_SETTLE=0 "$ROOT/bin/fm-bootstrap.sh" 2>/dev/null)
  out2=$(printf '%s\n' "$out2" | grep -E '^(BOOTSTRAP_INFO: nudged|NUDGE_SECONDMATES:)' || true)
  [ -z "$out2" ] || fail "idempotent retry should converge to no nudge output, got: $out2"
